...
import com.ams.mufins.webservice.model.IndicatorTahunModel;
import com.ams.mufins.webservice.util.CommonApiUtil;

public class GetIndicatorTahun extends Action {
	Logger log = Logger.getLogger(this.getClass());

	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String action = mapping.getParameter();
		if ("GETINDICATORTAHUN".equalsIgnoreCase(action)) {
			return this.performGetIndicatorTahun(mapping, form, request, response);
		}
		return null;
	}

	@SuppressWarnings("unchecked")
	private ActionForward performGetIndicatorTahun(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request,
		...
		List<IndicatorTahunModel> list = null;

		try {
			session = UsersDAO.getInstance().getSession();
			String sql = "select indicator_tahun_id as id, batas_bawah as batasBawah, batas_atas as batasAtas, indicator_tahun as indicatorTahun "
					+ "from indicator_tahun";
			list = (List<IndicatorTahunModel>) session.createSQLQuery(sql)
					...
					.setResultTransformer(Transformers.aliasToBean(IndicatorTahunModel.class))
					.list();
		} catch (Exception e) {
			e.printStackTrace();
			log.error(e);
	...
	private ActionForward returnResponse(HttpServletResponse response, String responseMessage, int responseStatus, List<IndicatorTahunModel> list) {
		try {
			...
			for(IndicatorTahunModel model : list) {
