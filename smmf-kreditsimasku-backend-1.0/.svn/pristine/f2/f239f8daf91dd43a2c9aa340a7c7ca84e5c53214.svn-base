...
import com.ams.mufins.webservice.model.RateAsuransiMotor;
import com.ams.mufins.webservice.util.CommonApiUtil;

public class GetRateAsuransiMotor extends Action {
	Logger log = Logger.getLogger(this.getClass());

	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String action = mapping.getParameter();
		if ("GETRATEASURANSIMOTOR".equalsIgnoreCase(action)) {
			return this.performGetRateAsuransiMotor(mapping, form, request, response);
		}
		return null;
	}

	@SuppressWarnings("unchecked")
	private ActionForward performGetRateAsuransiMotor(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request,
		...
		List<RateAsuransiMotor> list= null;
		
		try {
			session = UsersDAO.getInstance().getSession();
			String sql = "select rate_asuransi_motor_id as id, produk, tipe, tenor, value_asuransi as valueAsuransi "
					+ "from rate_asuransi_motor";
			list = (List<RateAsuransiMotor>) session.createSQLQuery(sql)
					.addScalar("id", Hibernate.LONG)
					.addScalar("produk", Hibernate.STRING)
					.addScalar("tipe", Hibernate.STRING)
					.addScalar("tenor", Hibernate.INTEGER)
					.addScalar("valueAsuransi", Hibernate.INTEGER)
					.setResultTransformer(Transformers.aliasToBean(RateAsuransiMotor.class))
					.list();
		} catch (Exception e) {
			e.printStackTrace();
			log.error(e);
	...
	private ActionForward returnResponse(HttpServletResponse response, String responseMessage, int responseStatus, List<RateAsuransiMotor> list) {
		try {
			...
			for(RateAsuransiMotor model : list) {
