...
public class GetResetPassword extends Action {
	Logger log = Logger.getLogger(this.getClass());

	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String action = mapping.getParameter();
		if ("GETRESETPASSWORD".equalsIgnoreCase(action)) {
			return this.performGetResetPassword(mapping, form, request, response);
		}
		return null;
	}

	@SuppressWarnings({ "unused" })
	private ActionForward performGetResetPassword(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request,
			...
			session = CustomerMobileDAO.getInstance().getSession();
				...
				if (!objJson.getString("handphone").equalsIgnoreCase("")) {
					...
					if(cm.getDeviceId().equalsIgnoreCase(objJson.getString("deviceId"))) {
						...
						String hashPassword = CommonUtil.hashMufins(CommonUtil.decryptString(objJson.getString("password"), 
								objJson.getString("key")));
						...
						session.createSQLQuery(sqlUpdate)
							...
							.setString("hashPassword", hashPassword)
							.executeUpdate();
						...
						responseMessage = "Tidak Boleh Reset Password Pada HP Yang Berbeda.";
						return returnResponse(response, responseMessage, responseStatus, null);
					}
