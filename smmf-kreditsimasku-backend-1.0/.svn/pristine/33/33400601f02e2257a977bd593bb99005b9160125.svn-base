...
import com.ams.mufins.webservice.model.RateAsuransiMobilTlo;
import com.ams.mufins.webservice.util.CommonApiUtil;

public class GetRateAsuransiMobilTlo extends Action {
	Logger log = Logger.getLogger(this.getClass());

	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String action = mapping.getParameter();
		if ("GETRATEASURANSIMOBILTLO".equalsIgnoreCase(action)) {
			return this.performGetRateAsuransiMobilTlo(mapping, form, request, response);
		}
		return null;
	}

	@SuppressWarnings("unchecked")
	private ActionForward performGetRateAsuransiMobilTlo(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request,
		...
		List<RateAsuransiMobilTlo> list= null;
		
		try {
			session = UsersDAO.getInstance().getSession();
			String sql = "select rate_asuransi_mobil_tlo_id as id, produk, tipe, tenor, value_asuransi as valueAsuransi "
					+ "from rate_asuransi_mobil_tlo";
			list = (List<RateAsuransiMobilTlo>) session.createSQLQuery(sql)
					.addScalar("id", Hibernate.LONG)
					.addScalar("produk", Hibernate.STRING)
					.addScalar("tipe", Hibernate.STRING)
					.addScalar("tenor", Hibernate.INTEGER)
					.addScalar("valueAsuransi", Hibernate.DOUBLE)
					.setResultTransformer(Transformers.aliasToBean(RateAsuransiMobilTlo.class))
					.list();
		} catch (Exception e) {
			e.printStackTrace();
			log.error(e);
	...
	private ActionForward returnResponse(HttpServletResponse response, String responseMessage, int responseStatus, List<RateAsuransiMobilTlo> list) {
		try {
			...
			for(RateAsuransiMobilTlo model : list) {
				...
				objectBuilder.add("value_asuransi", model.getValueAsuransi() > 0 ? String.valueOf(model.getValueAsuransi()) : "0");
