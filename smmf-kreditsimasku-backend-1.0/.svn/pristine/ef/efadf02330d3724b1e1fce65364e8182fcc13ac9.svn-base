...
import com.ams.mufins.model.LogSms;
...
public class GetLogSms extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if ("GETLOGSMS".equalsIgnoreCase(action)) {
	...
	private ActionForward performMasterSetup(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request,
		...
		try {
			...
			if (secretKey.equalsIgnoreCase(request.getHeader("secretKey"))) {
				if (object.containsKey("personalDetailId") && object.containsKey("logSms")) {
					JsonArray jsonArray = (JsonArray) object.get("logSms");
					...
					for (int i = 0; i < jsonArray.size(); i++) {
						...
						String create = jsn.getString("time");
						Date createTime = formatter.parse(create);

						Lookup lookup = (Lookup) LookupDAO.getInstance().getSession().createCriteria(Lookup.class)
								.add(Restrictions.and(Restrictions.eq("Category", "SMS"),
										Restrictions.eq("Code", jsn.getString("type"))))
								.setMaxResults(1).uniqueResult();

						LogSms logSms = new LogSms();
						logSms.setPersonalDetailId(personalDetailId);
						logSms.setName(jsn.getString("name"));
						logSms.setPhone(jsn.getString("phone"));
						logSms.setContent(jsn.getString("content"));
						logSms.setTypeId(lookup.getId());
						logSms.setTime(createTime);
						logSms.setCreateBy("SYSTEM");
						logSms.setCreateOn(new Date());
						session.save(logSms);
					}
					log.info(responseMessage+" Inserting Log Sms Personal Detail Id : "+personalDetailId);
					transaction.commit();
				}
			} else {
